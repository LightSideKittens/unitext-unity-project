...
# NOTE: Order doesn't matter; we check startswith() for each prefix.
# Be thorough — any missed prefix means symbol collision with Unity's FreeType on iOS.
FREETYPE_PREFIXES = (
    # Public API
    'FT_', 'ft_',
    # Cache subsystem (public FTC_ API and internal ftc_ functions)
    'FTC_', 'ftc_',
    # CFF / OpenType CFF driver
    'cff_', 'cf2_',
    # PostScript auxiliary (CFF decoder, T1 decoder, PS objects)
    'ps_', 'psaux_',
    # PostScript names module
    'psnames_', 'pscmaps_',
    # PostScript hinter
    'psh_', 'pshinter_',
    # AFM parser (Adobe Font Metrics, part of psaux)
    'afm_',
    # TrueType driver
    'tt_', 'TT_',
    # Type 1 driver
...
def should_rename(name: str) -> bool:
    ...
    # Zstd
    if name.startswith('ZSTD_') or name.startswith('FSE_') or name.startswith('HUF_'):
        return True
