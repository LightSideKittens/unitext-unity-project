...
public class GoldenFileTestRunner : MonoBehaviour
{
    ...
    static void OnRuntimeStart()
    {
        ...
        Debug.unityLogger.Log("[GoldenFileTestRunner] Starting test runner...");
    ...
    void ProcessSnapshot(string testName, MeshDataSnapshot snapshot, TestMode mode, DateTime startTime, UniText uniText)
    {
        ...
        else
        {
            ...
            if (golden == null)
            {
                ...
                Debug.LogWarning($"  <color=red>✗ {testName}: Golden file not found</color>");
                return;
            }
            ...
            if (passed)
                ...
                Debug.LogWarning($"  <color=red>✗ {testName}</color>: {errorMsg}");
        }
    }
    ...
    void AddMeshToSnapshot(Mesh mesh, MeshDataSnapshot snapshot,
        UniText.TestSegmentFontInfo fontInfo, UniTextFontProvider fontProvider)
    {
        if (mesh == null) return;
        ...
        UniText.TestSegmentFontInfo fontInfo, UniTextFontProvider fontProvider)
    {
        ...
        int atlasSize = font is EmojiFont ef ? ef.AtlasSize : 0;
        int padding = font != null ? font.AtlasPadding : 0;
        Dictionary<long, Glyph> glyphTable = font?.GlyphLookupTable;

        bool isEmoji = atlasSize > 0;
        var primitives = FindPrimitives(segment.triangles, vertexCount);

        for (int i = 0; i < vertexCount; i++)
        {
            var uv = segment.uvs[i];
            if (!isEmoji)
            {
                uv = new SerializableVector4(new Vector4(uv.x, uv.y, 0f, uv.w));
            }
            segment.stableUVs.Add(uv);
        }
    ...
    static int IdentifyGlyphFromUVs(Dictionary<long, Glyph> glyphTable, int atlasSize, int padding,
    ...
    static void NormalizeUVs(List<SerializableVector4> stableUVs,
        ...
        GlyphRect rect, int atlasSize, int padding)
    {
        float invAtlas = 1f / atlasSize;
        float gMinX = (rect.x - padding) * invAtlas;
        float gMinY = (rect.y - padding) * invAtlas;
    ...
    List<string> VerifyActualUVsAgainstFont(MeshDataSnapshot actual, UniText uniText)
    {
        ...
        for (int s = 0; s < actual.segments.Count; s++)
        {
            ...
            int atlasSize = font is EmojiFont ef2 ? ef2.AtlasSize : 0;
